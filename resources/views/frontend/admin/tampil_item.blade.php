                ...
                <div
                    ...
                    @if (Auth::user()->role == 'admin')
                        <div class="d-grid d-md-flex justify-content-md-end" style="gap: 5px">
                            <a href="{{ route('admin.crud-tambah') }}" class="btn btn-success bi bi-file-earmark-plus-fill">
                                New Item</a>
                            <a href="{{ route('admin.generate_pdf') }}"
                                class="btn btn-primary bi bi-file-earmark-arrow-down position-relative">Download Data</a>
                        </div>
                    @endif
                </div>
                ...
                <tbody>
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->SKU }}</td>
                            <td><img src="{{ asset('storage/' . $item->gambar) }}" alt="Gambar {{ $item->name }}"
                                    width="50"></td>
